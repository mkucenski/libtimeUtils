...
int32_t getUnix32FromPTime(const posix_time::ptime& pt) {
	...
	if (!td_fromUnixEpoch.is_negative()) {
		return td_fromUnixEpoch.total_seconds();	//Return the number of seconds in the duration between the given time and the UNIX epoch (1/1/1970)
	}
	return 0;
}
...
posix_time::ptime getFromWindows64(int64_t t) {
	...
	DEBUG_INFO("totalsec: " << t_totalsecs << " hours: " << t_hours << " mins: " << t_mins << " secs: " << t_secs);
