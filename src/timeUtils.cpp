...
string getLocalDateTimeString(const boost::local_time::local_date_time& ldt, const string& strfmt) {
	...
	stringstream ss;
	try {
		boost::local_time::local_time_facet* output_facet = new boost::local_time::local_time_facet(strfmt.c_str());
		ss.imbue(locale(ss.getloc(), output_facet));
		ss << ldt;
	} catch (...) {
		ERROR("getLocalDateTimeString() - Unknown exception");
	}
	return ss.str();
}
...
int32_t getUnix32FromPTime(const boost::posix_time::ptime& pt) {
	...
	try {
		boost::posix_time::time_duration td_fromUnixEpoch = pt - boost::posix_time::ptime(boost::gregorian::date(1970, 1, 1), boost::posix_time::time_duration(0, 0, 0));
		...
		ERROR("getUnix32FromPTime() - Unknown exception");
	}
...
boost::posix_time::ptime getFromWindows64(int64_t t) {
	...
	try {
		rv = boost::posix_time::ptime(boost::gregorian::date(1601, 1, 1), boost::posix_time::time_duration(t_hours, t_mins, t_secs));
	} catch (...) {
		ERROR("getFromWindows64() - Unknown exception");
	}
